...
import { approvePayment } from "@/app/actions/reservations";
...
export default function AdminClientActions({ type, id, paymentProofUrl }: AdminClientActionsProps) {
  ...
  const handleReject = async () => {
    if (type !== "user") return; // We only implemented reject for users
        ...
        <div className="flex gap-2">
          {type === "user" && (
            <button
              ...
              disabled={loading}
              className="inline-flex items-center p-2 border border-transparent rounded-full shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-red-500 disabled:opacity-50 transition-colors cursor-pointer"
              title="Reject User"
            >
          ...
          <button
            ...
            title={type === "user" ? "Approve User" : "Approve Payment"}
          >
